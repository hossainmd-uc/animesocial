...
async function importAnime(malId: number) {
  try {
    ...
    if (animeData.theme) {
      ...
      if (animeData.theme.openings) {
        animeData.theme.openings.forEach(async (opening: string, index: number) => {
      ...
      if (animeData.theme.endings) {
        animeData.theme.endings.forEach(async (ending: string, index: number) => {
