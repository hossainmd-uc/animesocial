...
function getImportStats(progress) {
  const now = new Date();
...
async function importAnime(malId) {
  try {
    ...
    // Import related data (genres, studios, etc.)
    if (animeData.genres) {
...
async function testImport() {
  ...
  // Popular anime IDs for testing
  const testIds = [1, 5, 16, 20, 30, 11, 12, 13, 14, 15]; 
  ...
  for (let i = 0; i < testIds.length; i++) {
    ...
    if (!progress.processedIds.includes(malId)) {
      ...
      console.log(`⚠️  Already processed MAL ID ${malId}, skipping...`);
    }
  }
