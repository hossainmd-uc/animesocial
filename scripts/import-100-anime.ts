...
function determineSeriesType(anime: any): string {
...
function determineSeriesOrder(anime: any): number {
  const title = anime.title.toLowerCase();
...
async function import100Anime() {
  ...
  try {
    ...
    console.log(`📊 Found ${animeList.length} top anime to import\n`);
    ...
    for (let i = 0; i < animeList.length; i++) {
      ...
      console.log(`\n[${i + 1}/${animeList.length}] Processing: ${anime.title}`);
