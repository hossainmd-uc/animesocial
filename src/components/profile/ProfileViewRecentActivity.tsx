...
interface ProfileViewRecentActivityProps {
  username: string;
}

export default function ProfileViewRecentActivity({ username }: ProfileViewRecentActivityProps) {
  ...
  const fetchActivities = async () => {
    try {
      const response = await fetch(`/api/user/activities?username=${username}&limit=5`);
      if (response.ok) {
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {activities.length === 0 ? (
          <div className={`text-center py-8 flex-1 flex flex-col justify-center ${
            ...
            <p className="text-xs mt-1">This user hasn&apos;t tracked any anime activity yet!</p>
          </div>
        ) : (
