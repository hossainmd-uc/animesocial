...
interface ProfileViewAnimeStatsProps {
  username: string;
}

export default function ProfileViewAnimeStats({ username }: ProfileViewAnimeStatsProps) {
    ...
    <div className={`p-6 h-96 flex flex-col transition-all duration-300 ${
      ...
      {totalAnime === 0 ? (
        <div className={`text-center py-8 flex-1 flex flex-col justify-center ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          ...
          <p className="text-xs mt-1">This user hasn&apos;t added any anime to their list!</p>
        </div>
      ) : (
